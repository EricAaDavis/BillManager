...
class BillDetailTableViewController: UITableViewController, UITextFieldDelegate {
    ...
    @IBAction func remindSwitchChanged(_ sender: UISwitch) {
        ...
        if sender.isOn {
            /*
            bill.scheduleReminder(date: remindDatePicker.date, completion: { (bill) in
                print(bill)
    ...
    func presentNeedAuthorizationAlert() {
        let alert = UIAlertController(title: "Notifications denied", message: "We need your permission to alert you about due dates for your bills. To give us access, head over to the iOS Settings app and grant us access", preferredStyle: .alert)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if remindSwitch.isOn {
            ...
            bill.scheduleReminder(date: remindDatePicker.date) { (closureBill) in
            
                Database.shared.updateAndSave(closureBill)
                
            }
        } else {
            bill.remindDate = nil
            bill.unscheduleReminder()
